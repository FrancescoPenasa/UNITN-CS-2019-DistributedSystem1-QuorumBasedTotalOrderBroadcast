...
class Replica extends AbstractActor {
	...
	public enum Timeout {
		HEARTBEAT, DECISION, VOTE, ELECTION
	}
	...
	private void onJoinGroupMsg(JoinGroupMsg msg) {
		...
		logger.info("Replica " + this.id + " : joins in a group of " + this.replicas.size() + " peers");
	...
	// --- ReadRequest --- //
	/*
	 * ReadRequest and onReadRequest manage the read request from the client and
	 * send back the value.
	 */
	...
	private void onReadRequest(ReadRequest req) {
		...
		getContext().system().scheduler().scheduleOnce(Duration.create(getMsgDelay(), TimeUnit.MILLISECONDS),
				getSender(), new Client.ReadResponse(v), getContext().system().dispatcher(), getSelf());
		logger.info("Replica " + this.id + " : receives read req from " + getSender().path().name());
	}
	// ------------------- //

	// === WriteRequest === //
	/*
	 * WriteRequest manage update request from the client, if the receiving replica
	 * is the coordinator send UPDATE message with the new value to all other
	 * replicas if the receiving replica is not the coordinator send the UPDATE
	 * request to the coordinator (part of the update protocol)
	 */
	...
	private void onWriteRequest(WriteRequest req) {
		...
		if (isCoordinator()) {
			...
			logger.info("Replica " + this.id + " : receives write req from " + getSender().path().name());
			...
			logger.info("Replica " + this.id + " : receives write req from " + getSender().path().name());
			logger.info("Replica " + this.id + " : sends write req to coordinator" + this.coordinator);
	...
	// === VoteRequest === //
	/*
	 * All non-coordinator replicas will receive a VoteRequest and will answer with
	 * a VoteReponse
	 */
	...
	private void onVoteRequest(VoteRequest req) {
		if (crashed) {
			...
			logger.info("Replica " + this.id + " : receives vote req from " + getSender().path().name());
			logger.info("Replica " + this.id + " : sends its vote response to coordinator");
	...
	// === VoteResponse === //
	/*
	 * The coordinator receive the replicas votes and decide whether to push COMMIT
	 * or ABORT
	 */
	...
	private void onVoteResponse(VoteResponse res) {
		...
		logger.info("Replica " + this.id + " : receives vote response from " + getSender().path().name());
		if (hasDecided()) {
			return;
		}
		...
		if (v == Vote.YES) {
			...
			if (DEBUG && CRASH_ON_WRITEOKS_SEND && quorumReachedYes()) {
				...
				someVoters.remove(0);
				logger.info("Replica " + this.id + " : DEBUG send decision response to" + someVoters.toString());
	...
	private void onDecisionResponse(DecisionResponse res) {
		...
		if (res.decision == Decision.COMMIT) {
			...
			this.history.add(this.timeStamp);
			this.v = res.new_v;
			logger.info("Replica " + this.id + " : update " + this.epoch + ":" + this.sequence_number + " " + this.v);
		}
	...
	// === Timeouts === //
	/*
	 * Coordinator don't receive an ACK from a Replica after a VoteRequest, a
	 * replica has crashed.
	 */
	...
	public void onVoteTimeout(VoteTimeout msg) {
		if (!hasDecided() && isCoordinator()) {
			logger.info("Replica " + this.id + " : timeout on vote request, replica crashed");
	...
	/*
	 * Replica doesn't receive the decision from the coordinator, the coordinator
	 * might have crashed, ask the other replicas for the decision and start a new
	 * election
	 */
	...
	public void onDecisionTimeout(DecisionTimeout msg) {
		...
		if (!hasDecided() && !isCoordinator() && !electionStarted) {
			logger.info("Replica " + this.id + " : timeout on decision response, coordinator crashed");

			// start election process
			if (!electionStarted) {
				logger.info("Replica " + this.id + " : I'm starting the election");
	...
	/*
	 * Replica doesn't receive periodic HeartBeat from the coordinator, coordinator
	 * crashed, start a new Election
	 */
	...
	public void onHeartbeatTimeout(HeartbeatTimeout msg) {
		if (checkHeartbeat() && electionStarted == false) {
			logger.info("Replica " + this.id + " : timeout on heartbeat, coordinator crashed");
			try {
			...
			if (!electionStarted) {
				...
				logger.info("Replica " + this.id + " : election in progress");
			}
		}
	}
	...
	public void onElectionTimeout(ElectionTimeout msg) {
		...
		logger.info("Replica " + this.id + " : ack message not received, send election message to another replica");
	...
	private void onCoordinatorElectionMessage(CoordinatorElectionMessage msg) {
		logger.info("Replica " + this.id + " : receive coordinator election message from replica: " + msg.idReplica);
		if (DEBUG && CRASH_ON_ELECTION) {
			if (getID() == 2) { 
				logger.info("Replica " + this.id + " : crashed during election: " + getID());
				crash();
				return;
			}
		}
		...
		if (electionMessageReceived) {
			...
			// if there are more than one replica with the last update, find the highest id
			// of replica
			int maxID = 0;
			if (countMax > 1) {
			...
			if (id == indexMax) {
				...
				logger.info("Replica " + this.id + " : is elected as new coordinator");
				logger.info("Replica " + this.id + " : replicas indexes without last update: " + indexesOfReplicaWithoutUpdate.toString());
	...
	private void onCoordinatorElectionMessageACK(CoordinatorElectionMessageACK msg) {
		...
		logger.info("Replica " + this.id + " : message to: " + getSender().path().name() + " acknowledged");
	}
	...
	private void onSynchronizationMessage(SynchronizationMessage msg) {
		...
		logger.info("Replica " + this.id + " : sets new coordinator as: " + getSender().path().name());
	...
	public void crash() {
		...
		logger.info("Replica " + this.id + " : is crashed");
	...
	private void multicast(Serializable m) {
		for (ActorRef p : replicas) {
			if (p.equals(getSelf())) { // so the coordinator will not send it to himself
				...
				logger.info("Replica " + this.id + " : multicast sent to " + p.path().name());
				p.tell(m, getSelf());
			}
		}
	}
	...
	private void sendBeat() {
		logger.info("Replica " + this.id + " : sends first heartbeat");
		for (ActorRef p : replicas) {
			if (p.equals(getSelf())) { // so the coordinator will not send it to himself
			} else {
	...
	private void updateReplicas(Serializable m) {
		logger.info("Replica " + this.id + " : updating replicas");
		for (ActorRef p : replicas) {
			logger.info("Replica " + this.id + " : multicast sent to " + p.path().name());
			p.tell(m, getSelf());
		}
	...
	void setTimeout(Timeout t) {
		logger.info("Replica " + this.id + " : "+t.toString() + " timeout starts");
		if (t.equals(Timeout.HEARTBEAT)) {
			getContext().system().scheduler().scheduleOnce(Duration.create(TIMEOUT_HEARTBEAT, TimeUnit.MILLISECONDS),
					getSelf(), new HeartbeatTimeout(), getContext().system().dispatcher(), getSelf());
		} else if (t.equals(Timeout.VOTE)) {
			getContext().system().scheduler().scheduleOnce(Duration.create(TIMEOUT_VOTE, TimeUnit.MILLISECONDS),
					getSelf(), new VoteTimeout(), getContext().system().dispatcher(), getSelf());
			return;
		} else if (t.equals(Timeout.DECISION)) {
			getContext().system().scheduler().scheduleOnce(Duration.create(TIMEOUT_DECISION, TimeUnit.MILLISECONDS),
					getSelf(), new DecisionTimeout(), getContext().system().dispatcher(), getSelf());
			return;
		} else if (t.equals(Timeout.ELECTION)) {
			getContext().system().scheduler().scheduleOnce(Duration.create(TIMEOUT_ELECTION, TimeUnit.MILLISECONDS),
					getSelf(), new ElectionTimeout(), getContext().system().dispatcher(), getSelf());
		} else {
		}
	}

	boolean quorumReachedYes() {
		logger.info("Replica " + this.id + " : checks quorum");
	...
	public Receive createReceive() {
		return receiveBuilder()
				// both
				.match(JoinGroupMsg.class, this::onJoinGroupMsg).match(ReadRequest.class, this::onReadRequest)
				.match(WriteRequest.class, this::onWriteRequest).match(Crashed.class, this::onCrashed)
				...
				// only coordinator
				.match(VoteResponse.class, this::onVoteResponse).match(DecisionResponse.class, this::onDecisionResponse)

				// timeouts
				.match(ElectionTimeout.class, this::onElectionTimeout).match(VoteTimeout.class, this::onVoteTimeout)
